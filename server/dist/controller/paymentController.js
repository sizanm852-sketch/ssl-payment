...
exports.refundQueryController = exports.paymentCancel = exports.paymentFailed = exports.paymentSuccess = exports.initiatePayment = void 0;
...
const initiatePayment = async (req, res) => {
    ...
    try {
        ...
        const sslcz = new sslcommerz_lts_1.default(storeId, storePass, isLive);
        // Save pending transaction to database
        await model_1.Transaction.create({
...
const paymentSuccess = async (req, res) => {
    try {
        ...
        if (validationResponse.status === 'VALID' || validationResponse.status === 'VALIDATED') {
            await model_1.Transaction.findOneAndUpdate({ tran_id }, { status: 'SUCCESS', val_id });
            res.redirect(`http://localhost:5173/success/${tran_id}`);
        }
        else {
            // If validation fails, mark as failed
            await model_1.Transaction.findOneAndUpdate({ tran_id }, { status: 'FAILED' });
...
const paymentCancel = async (req, res) => {
    try {
        const { tran_id } = req.body;
        await model_1.Transaction.findOneAndUpdate({ tran_id }, { status: 'CANCELLED' });
...
const refundQueryController = async (req, res) => {
    try {
...
exports.refundQueryController = refundQueryController;
